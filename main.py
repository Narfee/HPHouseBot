...
client = commands.Bot(command_prefix=PREFIX, case_insensitive=True) # initates bot and sets prefix
client.remove_command('help') # removes the default help command so I can add my own
...
def get_members():
  ...
  with open("house_points.json" , "r") as file: 
    ...
    for house in member_dict.keys(): # for every house
      members += list(member_dict[house].keys()) # generates a list of members of the house and adds it
    return members
...
async def point_logger(): 
  ...
  # sends embed
  log_channel = client.get_channel(741447867088109718)
...
async def channel(ctx, channel, amount:int = 1): 
  if channel == "total": 
    ...
    c = client.get_channel(741445244838477874) 
    ...
    if points in range(1,1000): 
      with open("house_points.json" , "r+") as file: 
        ...
        if house: # if they have a house
          member_dict[house][str(user.id)]["points"] += int(points) # adds the points to the dict
    ...
    if points in range(1,1000): 
      with open("house_points.json" , "r+") as file: 
        ...
        if house: # if the user has a house
          member_dict[house][str(user.id)]["points"] -= int(points) # removes points
      ...
      if house: # if they have a house
        with open("house_points.json" , "r+") as file: 
          ...
          if member_dict[house][str(message.author.id)]["msgs"] >= MESSAGES_UNTIL_EMBED: 
            # gives points 
            member_dict[house][str(message.author.id)]["points"] += POINTS_FOR_MESSAGES 
